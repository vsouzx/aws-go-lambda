...
//PATCH /transaction
resource "aws_api_gateway_method" "transaction_gw_api_method_patch" {
    authorization   = "NONE"
    http_method     = "PATCH"
    resource_id     = aws_api_gateway_resource.transaction_gw_api_resource.id
    rest_api_id     = aws_api_gateway_rest_api.transaction_gw_api.id
...
resource "aws_api_gateway_integration" "transaction_lambda_integration_patch" {
    http_method = aws_api_gateway_method.transaction_gw_api_method_patch.http_method
    resource_id = aws_api_gateway_resource.transaction_gw_api_resource.id
    rest_api_id = aws_api_gateway_rest_api.transaction_gw_api.id
    type        = "AWS_PROXY"
...
resource "aws_api_gateway_method_response" "transaction_response_200_patch" {
  http_method = aws_api_gateway_method.transaction_gw_api_method_patch.http_method
  resource_id = aws_api_gateway_resource.transaction_gw_api_resource.id
  rest_api_id = aws_api_gateway_rest_api.transaction_gw_api.id
  status_code = "200"
